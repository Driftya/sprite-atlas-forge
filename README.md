...
Phases 0–3 are implemented. The deterministic untrimmed repacker and Phaser exporter are also working. The Windows MAUI client supports open/detect/save/save-as, editable sprite regions, a zoomable/pannable atlas canvas with direct sprite selection and drag handles, click/drag plus numeric connector editing, dirty-state protection, undo/redo, validation, cancellable progress, repacking, and Phaser export.
...
The MAUI sprite panel also supports manual recovery: **Add sprite** creates and selects a centered region with a unique `sprite_NNN` ID, which can then be corrected through **Source region**. Sprites can be selected directly on the image; the selected source region exposes eight drag handles for exact edge/corner resizing, with the numeric editor as a fallback. **Save selected sprite as PNG** writes a lossless crop of the displayed atlas frame through the Windows Save dialog. **Delete selected** removes the current region. These operations participate in undo/redo; source-region resizing and adding sprites to an already repacked atlas are intentionally blocked. Sprite bounds are rendered in one lightweight drawing surface, so changing zoom does not recreate a native control and label for every sprite.
