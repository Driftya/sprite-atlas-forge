...
[![Tests](https://img.shields.io/badge/tests-109%2F109%20passed-brightgreen)](https://github.com/Driftya/sprite-atlas-forge/actions/workflows/ci.yml)
...
**Recover detached details** is opt-in for Auto mode. It performs a third pass over the original low-alpha silhouette and expands a detected sprite with nearby detail pixels assigned to their nearest unambiguous sprite within sixteen pixels. This recovers faint generated ornaments that Auto's learned alpha cutoff intentionally excludes from its main pass, while tied pixels between sprites remain excluded. The same recovery can be applied to only the selected sprite with **Recover detached details** in the sprite actions. The CLI equivalent for whole-sheet detection is `--recover-detached-details`.
